    ...
    <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-[#0b1c2a]/70 bg-[#0b1c2a]/90 text-[#f7f3ea] border-b border-white/10">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} className="text-[#f7f3ea]/90 hover:text-[#c7a97b] transition-colors text-sm">
              {item.label}
            </a>
          ))}
          <a href="#menu" className="ml-2 inline-flex items-center rounded-full bg-[#c7a97b] text-[#0b1c2a] px-4 py-2 text-sm font-medium shadow hover:shadow-md transition-shadow">View Menu</a>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-[#0b1c2a]">
          <div className="max-w-6xl mx-auto px-4 py-3 grid gap-2">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setOpen(false)} className="text-[#f7f3ea]/90 hover:text-[#c7a97b] py-2">
                {item.label}
              </a>
            ))}
            <a href="#menu" onClick={() => setOpen(false)} className="inline-flex items-center justify-center rounded-md bg-[#c7a97b] text-[#0b1c2a] px-4 py-2 text-sm font-medium">View Menu</a>
          </div>
        </div>
      )}
    </header>
